...
const smoothieMachine = (...ingredients) => {
  ...
  function smoothieSentence(...ingredients1) {
    allIngredients.push(ingredients1);

    let newSentence = "I'm having a smoothie with " + allIngredients[0];

    newSentence += allIngredients.slice(1).join(',').split(',').join(' and ');
